...
Проект полностью контейнеризован и запускается одной командой с помощью
**docker-compose**.

## 📦 Требования

-   Docker 20+
-   Docker Compose 1.29+ (или встроенный compose v2)
-   В корне проекта должны лежать файлы:
    -   `docker-compose.yaml`
    -   `Dockerfile`
    -   `.env`
...
Важно: внутри Docker база доступна по хосту **db**, а не localhost.
...
Команда выполнит автоматический запуск всех сервисов:
...
# 🔄 5. Пересборка проекта после изменения зависимостей

    docker-compose up -d --build

------------------------------------------------------------------------

CI/CD Workflow (GitHub Actions + Poetry)

Этот workflow автоматически запускает тесты и деплой при пуше в репозиторий.
Он состоит из двух job’ов: test и deploy.

🔍 test

• Запускается на Ubuntu
• Устанавливает Python
• Кэширует зависимости Poetry
• Устанавливает зависимости (poetry install)
• Выполняет миграции
• Запускает тесты

Используется Poetry, поэтому никаких pip install -r requirements.txt — зависимости берутся напрямую из pyproject.toml и poetry.lock.

🚀 deploy

• Запускается только если job test завершился успешно
• Подключается по SSH к серверу
• Копирует код через rsync
...
После этого приложение перезапускается через systemctl.
